...
| **Trigger steelman — multi-skill reuse in an unpredictable recurring env** | ✅ DONE 2026-06-14 (`p3_unpredictable.py`, `PREREG_P3_unpredictable.md`, N=30). Steelmans PAO to a skill *library* + change-point-triggered re-selection, in the one regime built to need it (random, unsignaled, recurring shifts) | **NO LEAD.** Pre-registered primary fires "LEAD FOUND" but it is a **metric artifact**: plain PPO wins on total reward / asymptotic competence / recovery; detection not load-bearing (Obs, no detection, scores best on the primary). The library is net-harmful. **Methodological result:** the Two-Gate "fast-recovery" metric is gameable. See `FINDINGS.md` | done |
| **P3 — PAO MVP (= .md P0+P1)** *(not started — Two-Gate investigation paused after 4 falsifications)* | Fork DreamerV3; MiniGrid 8×8; G_t + learnability filter + BOCPD + distill + validate; **PAO-AdmitAll vs Random-Window** | **H1 at scale** (proper env, real world model) | 4–5 weeks |
